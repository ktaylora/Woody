cat("## Mesquite Landcover Builder ###\n");
...
ACCEPTABLE_TYPES = c("grass","forest","water","row_crop","small_grains","mesq_high","mesq_low")
...
if( length(ARGV) > 0 ){
  ...
  TRAINING_DATA = paste("~/Incoming/offyear_study/vector/mesq_training_data/mesq_balanced_training_pts_2015_2017_",SCALE,"m.geojson",sep="")
}
...
if( DO_LOCAL_EXTRACTION ){
  ...
  explanatories <- raster::stack(paste("/home/ktaylora/mesq_study_naip_ndvi_2015_2017_",SCALE,"m.tif",sep=""))
  ...
  if( sum(grepl(colnames(training_data), pattern="ndvi")) > 0 ){
    ...
    GREENESS_INDEX <- "ndvi"
  } else if ( sum(grepl(colnames(training_data), pattern="gf")) > 0 ) {
    cat("DEBUG: Looks like we are using an alternative greeness index specified by the user (e.g., as 'gf_') \n")
    GREENESS_INDEX <- "gf"
  } else {
...
cat("DEBUG: Checking for OOB error convergence. See: 'figures/mesq_rf_oob_error_convergence.png'\n")
...
png("figures/mesq_rf_oob_error_convergence.png", width=800, height=600)
  plot(
...
# test$predicted <- ifelse(grepl(test$predicted, pattern="mesq"), yes="mesq", no="other")
# test$observed <- ifelse(grepl(test$observed, pattern="mesq"), yes="mesq", no="other")

cat("DEBUG: Building ROC curves. See: 'figures/mesq_roc_curve.png'\n")

# build a table of our per-class AUC values
roc_auc_values <- round(data.frame(
  mesq=suppressMessages(as.numeric(auc(roc(ifelse(grepl(test$observed, pattern="mesq"), yes="mesq", no="other"), as.numeric(test$mesq))))),
...
png("figures/mesq_roc_curve.png", width=800, height=600)
  
  plot(
    roc(ifelse(grepl(test$observed, pattern="mesq"), yes="mesq", no="other"), as.numeric(test$mesq)),
    col="#86b300", # olive
    lwd=1.25,
    cex.lab=1.75,
    cex.axis=1.75,
  ...
  legend(
    ...
    legend = c(
      paste("mesquite (AUC=",roc_auc_values$mesq,")", sep=""), 
...
write.csv(
  ...
  "products/rf_mesq_confusion_matrix.csv",
  row.names=F
)
